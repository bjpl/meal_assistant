...
export const MATCH_METHODS: Record<string, MatchMethod> = {
...
export class DealMatcher {
   ...
   */
  findMatchesForItem(item: ShoppingItem, deals: AdDeal[], minConfidence: number = 50): CandidateMatch[] {
   ...
   */
  calculateMatch(item: ShoppingItem, deal: AdDeal): MatchResult {
   ...
   */
  async applyCorrection(dealId: string, correction: Record<string, any>): Promise<{ success: boolean; correctionId: string; updatedFields: string[] }> {
   ...
   */
  async confirmMatch(matchId: string, confirmed: boolean = true): Promise<{ matchId: string; user_confirmed: boolean; confirmed_at: string }> {
    return {
      matchId,
   ...
   */
  autoApplyMatches(matches: DealMatch[], threshold: number = 90): { applied: DealMatch[]; pending: DealMatch[]; autoAppliedCount: number; pendingReviewCount: number } {
    const applied: DealMatch[] = [];
    const pending: DealMatch[] = [];
   ...
   */
  getCorrectionStats(templateId: string): { templateId: string; totalCorrections: number; fieldCounts: Record<string, number>; mostCorrectedFields: Array<{ field: string; count: number }> } {
    const templateCorrections = this.correctionHistory.filter(
      c => (c as any).templateId === templateId
    );
   ...
   */
  findDealsForComponent(componentId: string, deals: AdDeal[], componentName: string): CandidateMatch[] {
...
// Singleton instance
export const dealMatcher = new DealMatcher();
