...
const { createABTest, TemplateStatus } = require('./templateTypes');
const templateService = require('./templateService');

// In-memory storage for A/B tests
const abTestStore = new Map();
const activeTestsByStore = new Map(); // storeId -> activeTestId
...
class ABTestService {
   ...
   * @param {string} controlTemplateId - Control (original) template
   * @param {string} variantTemplateId - Variant (new) template
   * @param {Object} config - Test configuration
   * @returns {Promise<Object>} Created A/B test
   */
  async createTest(controlTemplateId, variantTemplateId, config = {}) {
  ...
  /**
   * Start an A/B test
   * @param {string} testId - Test to start
   * @returns {Promise<Object>} Started test
   */
  async startTest(testId) {
    ...
    test.store_id = test.store_id || control.store_id;
   ...
   * @param {string} storeId - Store ID for the ad
   * @returns {Promise<Object|null>} Routing result or null if no active test
   */
  async routeAd(storeId) {
   ...
   */
  async recordResult(testId, group, result) {
    const test = abTestStore.get(testId);
    if (!test) {
      throw new Error(`A/B test not found: ${testId}`);
    }
   ...
   * @param {string} testId - Test ID
   * @returns {Promise<boolean>} Whether test was concluded
   */
   ...
   * @param {string} testId - Test to conclude
   * @returns {Promise<Object>} Conclusion results
   */
  ...
  analyzeResults(test) {
    ...
    // In production, use proper statistical tests (z-test, chi-squared, etc.)
  ...
  /**
   * Cancel an A/B test
   * @param {string} testId - Test to cancel
   ...
   */
  async getTestsByStore(storeId, options = {}) {
   ...
   */
  async getActiveTest(storeId) {
    const testId = activeTestsByStore.get(storeId);
    return testId ? abTestStore.get(testId) : null;
  }

  /**
   * Get tests by user
   */
  async getTestsByUser(userId, options = {}) {
    const tests = Array.from(abTestStore.values())
   ...
   * @param {string} testId - Completed test ID
   * @param {number} targetPercentage - Target rollout percentage
   * @param {number} stepSize - Percentage increase per step
   * @returns {Promise<Object>} Rollout configuration
   */
  async createGradualRollout(testId, targetPercentage = 100, stepSize = 10) {
    const test = abTestStore.get(testId);
    if (!test) {
      throw new Error(`A/B test not found: ${testId}`);
    }
   ...
   */
  async executeRolloutStep(testId) {
    const test = abTestStore.get(testId);
...
module.exports.ABTestService = ABTestService;
module.exports.abTestStore = abTestStore;
